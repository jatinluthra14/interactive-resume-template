...
function create() {
  ...
  game.add.tween(corel).to({
    y: h - 450
  }, 2000, Phaser.Easing.Quadratic.InOut, true, 0, 2000, true);
  ...
  */
  ...
  game.add.text(5200, h - 70, '*Percentiles as calculated by HackerEarth', {
    font: "20px monospace",
    fill: "#fff"
  });
  ...
  game.add.text(base5 + 240, h - 260, "Achievements", {
    font: "25px Arial Black"
  });
  ...
  game.add.text(base6 + 335, h - 260, "Other Highlights", {
  ...
  game.add.text(base7 + 135, h - 260, "Online Profiles", {
    font: "20px Arial Black"
  });
  ...
  game.add.tween(football).to({
    y: h - 300
  }, 400, Phaser.Easing.Back.InOut, true, 0, 400, true);
...
function collisionHandler(bullet, tile) {
  ...
  if (explosion_count == 1) {
    ...
    game.add.tween(css).to({
      y: h - 550
    }, 300).start();
    game.add.tween(css.scale).to({
      y: 0.9,
      x: 0.9
    }, 500).start();
    ...
    game.add.tween(html).to({
      y: h - 550
    }, 300).start();
    game.add.tween(html.scale).to({
      y: 0.9,
      x: 0.9
    }, 500).start();
    ...
    game.add.tween(js).to({
      y: h - 550
    }, 300).start();
    game.add.tween(js.scale).to({
      y: 0.9,
      x: 0.9
    }, 800).start();
    ...
    game.add.tween(py).to({
      y: h - 550
    }, 300).start();
    game.add.tween(py.scale).to({
      y: 0.6,
      x: 0.6
    }, 500).start();
    ...
    */
    ...
    game.add.tween(andro).to({
      y: h - 550
    }, 300).start();
    game.add.tween(andro.scale).to({
      y: 0.6,
      x: 0.6
    }, 600).start();
...
function update() {
  ...
  if (hero.x > 15350) {
    ...
    game.add.tween(shakti).to({
      x: base5 + 600,
      y: h - 610
    }, 100).start();
    game.add.tween(shakti_cup).to({
      x: base5 + 500,
      y: h - 475
    }, 100).start();
  ...
  if (hero.x > platformLength - 350) {
    ...
    var t = game.add.tween(flag).to({
      y: h - 400
    }, 1000);
    t.start();
    t.onComplete.add(function() {
      game.add.text(platformLength - 185, h - 370, "Thanks for\n Visiting !", style_white);
    }, this);
  ...
  if (hero.x > base7 + 500) {
    game.add.tween(cloud_moving2).to({
      x: base7,
      alpha: 0
    }, 5000).start();
    game.add.tween(cloud_moving1).to({
      x: platformLength,
      alpha: 0
    }, 5000).start();
...
function fadeText() {
  ...
  game.add.tween(jstext).to({
    alpha: 1
  }, 2000, Phaser.Easing.Linear.None, true).start();
  game.add.tween(ctext).to({
    alpha: 1
  }, 2000, Phaser.Easing.Linear.None, true).start();
  game.add.tween(pythontext).to({
    alpha: 1
  }, 2000, Phaser.Easing.Linear.None, true).start();
  game.add.tween(javatext).to({
    alpha: 1
  }, 2000, Phaser.Easing.Linear.None, true).start();
}
